...
namespace AntMe.Player.first
{
    [Player(
        ColonyName = "first",
        FirstName = "Shyam Prasad Reddy",
        LastName = "Yenna"
    )]
    [Caste(
        Name = "collectors",
    ...
    public class firstClass : BaseAnt
    {
        #region Caste
        ...
        public override string ChooseCaste(Dictionary<string, int> typeCount)
        {
            ...
            else
            {
                ...
                return (float)typeCount["fighters"] / typeCount["collectors"] > 1.0f
                    ? "collectors" : "fighters";
            }
        ...
        public override void DestinationReached(Fruit fruit)
        {
            ...
            {
                MakeMark(20 - FriendlyAntsInViewrange, 200);
                Take(fruit);
                GoToAnthill();
            }
        }
        ...
        public override void SpotsEnemy(Bug bug)
        {
            if (Caste == "collectors")
            {
                // If the beetle is seen, it is determined whether the collector ant might collide,
                // if she continues straight ahead.
                int relativeDirection =
                    Coordinate.GetDegreesBetween(this, bug) - Direction;
                if (relativeDirection > -15 && relativeDirection < 15)
                {
                    Drop();
                    if (relativeDirection < 0)
                    {
                        TurnByDegrees(20 + relativeDirection);
                    }
                    else
                    {
                        TurnByDegrees(-20 - relativeDirection);
